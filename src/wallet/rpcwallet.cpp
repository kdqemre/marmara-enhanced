...
UniValue e_sendmany(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 2 || params.size() > 7) 
        throw runtime_error(
            "e_sendmany \"fromaccount\" [{\"address\":address,\"amount\":amount},...] ( minconf \"comment\" [\"address\",...] \"changeaddress\" fee )\n"
            "\nSend multiple times using an array of objects. Allows multiple outputs to the same address, a custom change address, and an optional fixed fee.\n"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) Must be set to the empty string \"\" to represent the default account.\n"
            "2. \"amounts\"           (array, required) A json array of objects with addresses and amounts\n"
            "    [\n"
            "      {\n"
            "        \"address\":\"address\", (string, required) The destination address\n"
            "        \"amount\":amount      (numeric, required) The numeric amount in MCL\n"
            "      }, ...\n"
            "    ]\n"
            "3. minconf               (numeric, optional, default=1) Only use balance confirmed at least this many times.\n"
            "4. \"comment\"           (string, optional) A comment\n"
            "5. subtractfeefromamount (array, optional) A json array with addresses. The fee will be equally deducted from the amount of each selected address.\n"
            "6. \"changeaddress\"     (string, optional) The destination address for the change generated by this transaction.\n"
            "7. fee                   (numeric, optional) A fixed transaction fee in MCL. If not set, the default fee is calculated.\n"
            "\nResult:\n"
            "\"transactionid\"        (string) The transaction id for the send. Only 1 transaction is created.\n"
            "\nExamples:\n"
            + HelpExampleCli("e_sendmany", "\"\" \"[{\\\"address\\\":\\\"RXAd...\\\",\\\"amount\\\":1.0}]\" 1 \"test\" \"[]\" \"RChangeAddr...\" 0.0001")
        );
    ...
    CFeeRate oldFee = payTxFee;
    CFeeRate oldMinRelay = ::minRelayTxFee;

    CAmount nUserTotalFee = 0;
    if (params.size() > 6 && !params[6].isNull()) {
        nUserTotalFee = AmountFromValue(params[6]);
    }

    string strAccount = AccountFromValue(params[0]);
    UniValue outputData = params[1].get_array();
    int nMinDepth = (params.size() > 2) ? params[2].get_int() : 1;
    CWalletTx wtx;
    if (params.size() > 3 && !params[3].isNull()) wtx.mapValue["comment"] = params[3].get_str();
    UniValue subFee = (params.size() > 4 && !params[4].isNull()) ? params[4].get_array() : UniValue(UniValue::VARR);

    CCoinControl coinControl;
    if (params.size() > 5 && !params[5].isNull() && !params[5].get_str().empty()) {
        coinControl.destChange = DecodeDestination(params[5].get_str());
    }

    std::vector<CRecipient> vecSend;
    for (size_t idx = 0; idx < outputData.size(); idx++) {
        const UniValue& output = outputData[idx];
        string addrStr = find_value(output, "address").get_str();
        CAmount nAmt = AmountFromValue(find_value(output, "amount"));
        bool fSub = false;
        for (size_t sidx = 0; sidx < subFee.size(); sidx++) {
            if (subFee[sidx].get_str() == addrStr) fSub = true;
        }
        vecSend.push_back({GetScriptForDestination(DecodeDestination(addrStr)), nAmt, fSub});
    }
    ...
    if (nUserTotalFee > 0) {
        ...
        if (pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRequired, nChangePosRet, strError, &coinControl)) {
            int64_t nTxSize = GetSerializeSize(wtx, SER_NETWORK, PROTOCOL_VERSION);
            
            payTxFee = CFeeRate((nUserTotalFee * 1000) / nTxSize);
        }
    }

    if (!pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRequired, nChangePosRet, strError, &coinControl)) {
        payTxFee = oldFee; ::minRelayTxFee = oldMinRelay;
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
    
    payTxFee = oldFee; 
    ::minRelayTxFee = oldMinRelay;
